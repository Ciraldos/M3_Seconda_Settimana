...
export class UserServiceService {
  ...
  constructor(private todoSrv:TodosServiceService) { }

  getArrTodosUsers(): iTodos[] {
    return this.todoSrv.todosArr.map((todo) => {
      const user = this.usersArr.find(
  ...
  getArrUsersTodos():iUser[] {
    return this.usersArr.map((users) => {
      const todo = this.todoSrv.todosArr.filter(
        (todo) => todo.userId == users.id
      );
      users.todos = todo;
      return users
      });
